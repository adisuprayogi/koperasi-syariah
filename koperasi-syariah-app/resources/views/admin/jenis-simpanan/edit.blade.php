...
<div class="max-w-4xl mx-auto">
    ...
    <h1 class="text-3xl font-bold text-gray-900 mt-4">Edit Jenis Simpanan</h1>
    <p class="text-gray-600 mt-2">Update jenis simpanan: {{ $jenisSimpanan->nama_simpanan }}</p>
</div>

<form action="{{ route('admin.jenis-simpanan.update', $jenisSimpanan->id) }}" method="POST">
    @csrf
    @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" id="kode_jenis" name="kode_jenis" required maxlength="10"
                           value="{{ old('kode_jenis', $jenisSimpanan->kode_jenis) }}"
                ...
                <div>
                    ...
                    <input type="text" id="nama_simpanan" name="nama_simpanan" required
                           value="{{ old('nama_simpanan', $jenisSimpanan->nama_simpanan) }}"
                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <select id="tipe_simpanan" name="tipe_simpanan" required
                        ...
                        <option value="modal" {{ old('tipe_simpanan', $jenisSimpanan->tipe_simpanan) == 'modal' ? 'selected' : '' }}>Simpanan Modal</option>
                        <option value="pokok" {{ old('tipe_simpanan', $jenisSimpanan->tipe_simpanan) == 'pokok' ? 'selected' : '' }}>Simpanan Pokok</option>
                        <option value="wajib" {{ old('tipe_simpanan', $jenisSimpanan->tipe_simpanan) == 'wajib' ? 'selected' : '' }}>Simpanan Wajib</option>
                        <option value="sukarela" {{ old('tipe_simpanan', $jenisSimpanan->tipe_simpanan) == 'sukarela' ? 'selected' : '' }}>Simpanan Sukarela</option>
                    </select>
                </div>
                <div>
                    ...
                    <select id="status" name="status" required
                        ...
                        <option value="1" {{ old('status', $jenisSimpanan->status) == '1' ? 'selected' : '' }}>Aktif</option>
                        <option value="0" {{ old('status', $jenisSimpanan->status) == '0' ? 'selected' : '' }}>Non-aktif</option>
                    </select>
                </div>
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select id="periode_hitung_bunga" name="periode_hitung_bunga" required
                        ...
                        <option value="bulanan" {{ old('periode_hitung_bunga', $jenisSimpanan->periode_hitung_bunga) == 'bulanan' ? 'selected' : '' }}>Bulanan</option>
                        <option value="tahunan" {{ old('periode_hitung_bunga', $jenisSimpanan->periode_hitung_bunga) == 'tahunan' ? 'selected' : '' }}>Tahunan</option>
                        <option value="otomatis" {{ old('periode_hitung_bunga', $jenisSimpanan->periode_hitung_bunga) == 'otomatis' ? 'selected' : '' }}>Otomatis (setiap transaksi)</option>
                        <option value="manual" {{ old('periode_hitung_bunga', $jenisSimpanan->periode_hitung_bunga) == 'manual' ? 'selected' : '' }}>Manual</option>
                    </select>
                </div>
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" id="minimal_setor" name="minimal_setor" required step="1000" min="0"
                           value="{{ old('minimal_setor', $jenisSimpanan->minimal_setor) }}"
                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                           placeholder="100000">
                ...
                <div>
                    ...
                    <input type="number" id="maksimal_setor" name="maksimal_setor" step="1000" min="0"
                           value="{{ old('maksimal_setor', $jenisSimpanan->maksimal_setor) }}"
                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
        ...
        <div class="px-6 py-4 space-y-4">
                ...
                <div class="flex space-x-4">
                    <label class="flex items-center">
                        <input type="radio" name="bisa_ditarik" value="1" required
                               {{ old('bisa_ditarik', $jenisSimpanan->bisa_ditarik) == '1' ? 'checked' : '' }}
                    ...
                    <label class="flex items-center">
                        <input type="radio" name="bisa_ditarik" value="0" required
                               {{ old('bisa_ditarik', $jenisSimpanan->bisa_ditarik) == '0' ? 'checked' : '' }}
                               class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300">
            ...
            <div>
                ...
                <textarea id="aturan_penarikan" name="aturan_penarikan" rows="3"
                          class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                          placeholder="Contoh: Penarikan minimal Rp 100.000, maksimal 50% dari total simpanan">{{ old('aturan_penarikan', $jenisSimpanan->aturan_penarikan) }}</textarea>
        ...
        <div class="px-6 py-4 space-y-4">
            <div>
                ...
                <textarea id="keterangan" name="keterangan" rows="2"
                          class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                          placeholder="Keterangan tambahan tentang jenis simpanan ini">{{ old('keterangan', $jenisSimpanan->keterangan) }}</textarea>
